...
def test_sum(a, b):
    # readin the executable path from the bin directory
    banary_path = os.path.join(os.path.dirname(__file__), "..", "bin", "sum")
    process = subprocess.Popen([banary_path], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,text=True)
